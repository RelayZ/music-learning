...
import {delListSong, listSongAdd, songOfSongId,listSongDetail,songOfSongName} from '../api/index';
...
export default {
    ...
    methods:{
        ...
        getData(){
            this.tempData = [];
            this.tableData = [];
            //从api/index.js里面可看到  传过来的是歌单的id，放在res里面
            listSongDetail(this.songListId).then(res => {
                for(let item of res){
                    this.getSong(item.songId);      //这个也就是后台的属性段名字songId
                }
            })
        },
